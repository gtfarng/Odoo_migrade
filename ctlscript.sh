...
if [ "x$1" = "xhelp" ] || [ "x$1" = "x" ]; then
    ...
    #odoo_gevent_code_start
        ...
        if test -x $ODOO_BACKGROUND_WORKER_SCRIPT; then
            $ODOO_BACKGROUND_WORKER_SCRIPT stop
            ODOO_BACKGROUND_WORKER_ERROR=$?
        fi
        #odoo_background_worker_code_end
    ...
    #odoo_gevent_code_start
        ...
        if test -x $ODOO_GEVENT_SCRIPT; then
            $ODOO_GEVENT_SCRIPT status
        fi
        #odoo_gevent_code_end
        #odoo_background_worker_code_start
        elif [ "x$2" = "xodoo_background_worker" ]; then
        if test -x $ODOO_BACKGROUND_WORKER_SCRIPT; then
            $ODOO_BACKGROUND_WORKER_SCRIPT status
        fi
        #odoo_background_worker_code_end
...
#odoo_gevent_code_start
    if [ "x$ODOO_GEVENT_ERROR" != "x" ] && [ $ODOO_GEVENT_ERROR -gt 0 ]; then
        ERROR=$ODOO_GEVENT_ERROR
    fi
    #odoo_gevent_code_end
    #odoo_background_worker_code_start
    if [ "x$ODOO_BACKGROUND_WORKER_ERROR" != "x" ] && [ $ODOO_BACKGROUND_WORKER_ERROR -gt 0 ]; then
        ERROR=$ODOO_BACKGROUND_WORKER_ERROR
    fi
    #odoo_background_worker_code_end
